...
/*   Updated: 2024/05/05 22:35:51 by maroy            ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
template <typename T> Array<T>::Array(const Array &anotherArr) : _arr_size(anotherArr.size()) {
    this->_arr = NULL;
    *this = anotherArr;
    std::cout << "Array copy constructor called" << std::endl;
}

template <typename T> Array<T> &Array<T>::operator=(const Array &anotherArr) {
    if (this->_arr)
        delete[] this->_arr;
    if (this != &anotherArr && anotherArr.size() > 0) {
        this->_arr_size = anotherArr.size();
        this->_arr = new T[this->_arr_size];
        for (unsigned int x = 0; x < this->size(); x++) {
            this->_arr[x] = anotherArr._arr[x];
        }
    }
